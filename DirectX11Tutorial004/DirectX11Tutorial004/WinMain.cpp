...
{
	// ウィンドウクラス登録
	TCHAR szWindowClass[] = "DIRECTX11 TUTORIAL004";	// ウィンドウクラスを識別する文字列
	WNDCLASSEX wcex;
...
HRESULT InitDevice()
{
	...
	D3D11_RASTERIZER_DESC descRast = {
		...
		// ピクセルの最大深度バイアス
		0.0f,
		// ピクセルの傾斜に基づいてバイアスをかける
		0.0f,
		// 距離に基づいてクリッピングを有効にする
		FALSE,
		// シザー矩形カリングを有効にする
		FALSE,
		// マルチサンプリングのアンチエイリアシングを有効にする
		FALSE,
		// 線のアンチエイリアシングを有効にする
		FALSE
	};

	// ラスタライザステージに動作を指示するラスタライザステートオブジェクトを作成
	g_pd3dDevice->CreateRasterizerState(
		// ラスタライザステートの記述へのポインタ
		&descRast,
		// 作成されるラスタライザステートオブジェクトへのポインタのアドレス
		&g_pRasterizerState
	);

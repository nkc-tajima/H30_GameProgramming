...
void CPolygon::Init(ID3D11Device* device, std::string vsStr, std::string psStr)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		// uv
		vertexList[i].uv[0] = i < 2 ? 0.0f : 1.0f;
		vertexList[i].uv[1] = i % 2 == 0 ? 1.0f : 0.0f;
	}
...
void CPolygon::Render(ID3D11DeviceContext* context, ID3D11Buffer* buffer, ID3D11ShaderResourceView* texture, ID3D11SamplerState* sampler)
{
	// サブリソースの更新
	context->UpdateSubresource(
		pVertexBuffer,
...
void CPolygon::SetUV(float cx, float cy, float nx, float ny)
{
	...
	vertexList[0].uv[0] = w * nx;
	vertexList[0].uv[1] = h * (ny + 1);

	vertexList[1].uv[0] = w * nx;
	vertexList[1].uv[1] = h * ny;

	vertexList[2].uv[0] = w * (nx + 1);
	vertexList[2].uv[1] = h * (ny + 1);

	vertexList[3].uv[0] = w * (nx + 1);
	vertexList[3].uv[1] = h * ny;
}
